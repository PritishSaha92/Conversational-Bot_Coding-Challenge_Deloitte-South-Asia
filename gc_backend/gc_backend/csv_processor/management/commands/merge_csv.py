...
def merge_files(activity_file, leave_file, onboarding_file, performance_file, rewards_file, vibemeter_file, output_file):
    ...
    master_df = (
        activity_summary
        .merge(leave_summary, on='Employee_ID', how='outer')
    ...
    # Drop unwanted column if needed
...
class Command(BaseCommand):
    help = "Merge all required HR CSV files from the 'media/csv_files' folder into a master CSV file."

    def add_arguments(self, parser):
        parser.add_argument(
            "--csv_folder",
            type=str,
            default="media/csv_files",  # <-- By default, use the media/csv_files folder
            help="Folder where all required CSV files are stored."
        )
        parser.add_argument(
            "--output",
            type=str,
            default="master_df.csv",
            help="Name of the output master CSV file (saved in the csv_folder)."
        )

    def handle(self, *args, **options):
        csv_folder = options["csv_folder"]
        output_file = os.path.join(csv_folder, options["output"])

        # Build full paths to each required file
        activity_file = os.path.join(csv_folder, "activity.csv")
        leave_file = os.path.join(csv_folder, "leave.csv")
        onboarding_file = os.path.join(csv_folder, "onboarding.csv")
        performance_file = os.path.join(csv_folder, "performance.csv")
        rewards_file = os.path.join(csv_folder, "rewards.csv")
        vibemeter_file = os.path.join(csv_folder, "vibemeter.csv")

        # Verify all required files exist
        required_files = [
            activity_file, leave_file, onboarding_file,
            performance_file, rewards_file, vibemeter_file
        ]
        missing_files = [f for f in required_files if not os.path.exists(f)]
        if missing_files:
            self.stderr.write("Missing the following required CSV files:")
            for file in missing_files:
                self.stderr.write(f" - {file}")
            return
        ...
        self.stdout.write(f"Master dataframe saved to: {output_file}")
